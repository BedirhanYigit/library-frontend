...
const BooksPage: React.FC = () => {
  ...
  const handleLoanBook = async (bookId: number) => {
    ...
    if (!userId)
      return setActionMessage({ text: 'Error: User ID missing. Please log in.', type: 'error' })

    try {
      const response = await fetch(
        `http://localhost:8080/create-loan-userId-bookId/${userId}/${bookId}`,
        { method: 'POST' },
      )
      if (response.ok) {
  ...
  const handleReserveBook = async (bookId: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/make-reservation/${userId}/${bookId}`, {
        method: 'POST',
      })
      if (response.ok) {
        setActionMessage({
