          ...
          <div className="books-grid">
            {reservations.map((item) => (
              <div key={item.id} className="book-card">
                ...
                <p className="book-detail"><strong>Author:</strong> {item.book?.author || item.author || 'N/A'}</p>
                <p className="book-detail"><strong>Genre:</strong> {item.book?.genre || item.genre || 'N/A'}</p>
                <p className="book-detail"><strong>ISBN:</strong> {item.book?.isbn || item.isbn || 'N/A'}</p>
                
                {/* RESERVATION DATE */}
                <p className="book-detail"><strong>Reserved On:</strong> {item.reservationDate || 'N/A'}</p>
                
                {/* STATUS INDICATOR */}
                <p className="book-status" style={{ color: '#f59e0b', fontWeight: 'bold', marginTop: '10px' }}>
                  WAITING FOR COPY
                </p>
